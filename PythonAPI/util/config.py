...
def main():
    ...
    if args.map is not None:
        ...
        if os.path.exists(args.osm_path):
            ...
            settings.generate_traffic_lights = True  # Optional: generate traffic lights

            # Convert OSM data to .xodr without set_osm_way_types
            xodr_data = carla.Osm2Odr.convert(data, settings)
            
            # Specify where to save the .xodr file
            xodr_path = "D:/CARLA_0.9.11/WindowsNoEditor/PythonAPI/util/opendrive/lucknow_fixed.xodr"
            with open(xodr_path, 'w') as xodr_file:
                xodr_file.write(xodr_data)
            
            print(f'OpenDRIVE map saved at: {xodr_path}')

            # Load the generated .xodr file into CARLA
            vertex_distance = 2.0  # in meters
            max_road_length = 500.0 # in meters
